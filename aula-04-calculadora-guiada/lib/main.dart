...
class _TelaCalculadoraState extends State<TelaCalculadora> {
  ...
  void _calcular() {
    final area = double.tryParse(_areaController.text.replaceAll(',', '.'));
    final prod = double.tryParse(
      _produtividadeController.text.replaceAll(',', '.'),
    );
    final cotacao = double.tryParse(
      _cotacaoController.text.replaceAll(',', '.'),
    );

    setState(() {
      if (area == null || prod == null || cotacao == null) {
        _erro = "Preencha os três campos com numeros válidos";
        _receita = null;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Receita da Lavoura -  Felipe Ferreira Gomes'),
        ...
        child: Column(
          ...
          children: [
            ...
            _CampoNumero(
              ...
              rotulo: "Produtividade (saca por ha)",
